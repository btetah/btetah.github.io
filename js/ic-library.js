...
var ICLibrary = {
    ...
    '7411': {
        ...
        simulate: function(pins) {
            pins[12] = (pins[1] && pins[2] && pins[13]) ? 1 : 0;  // 1Y = AND(1A,1B,1C)
            pins[6]  = (pins[3] && pins[4] && pins[5]) ? 1 : 0;   // 2Y = AND(2A,2B,2C)
            pins[8]  = (pins[9] && pins[10] && pins[11]) ? 1 : 0;  // 3Y = AND(3A,3B,3C)
            return pins;
        }
    },

    /* 74266 - Quad 2-Input XNOR Gate (Exclusive NOR) */
    '74266': {
        name: '74266',
        ...
        simulate: function(pins) {
            pins[3]  = (pins[1] ^ pins[2]) ? 0 : 1;    // 1Y = XNOR(1A,1B)
            pins[6]  = (pins[4] ^ pins[5]) ? 0 : 1;    // 2Y = XNOR(2A,2B)
            pins[8]  = (pins[9] ^ pins[10]) ? 0 : 1;   // 3Y = XNOR(3A,3B)
            pins[11] = (pins[12] ^ pins[13]) ? 0 : 1;   // 4Y = XNOR(4A,4B)
            return pins;
        }
    },
