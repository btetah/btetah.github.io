...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Pre-place default ICs in their fixed slots
    setTimeout(() => {
        const defaultICs = [
            { ic: '7400', slot: '1' },  // NAND  - IC-1 (top-left)
            { ic: '7402', slot: '2' },  // NOR   - IC-2
            { ic: '7408', slot: '3' },  // AND   - IC-3
            { ic: '7486', slot: '4' },  // XOR   - IC-4
            { ic: '7404', slot: '5' },  // NOT   - IC-5 (mid-left)
            { ic: '7432', slot: '8' },  // OR    - IC-8
            { ic: '7411', slot: '7' },  // AND3  - IC-7
        ];
        defaultICs.forEach(item => {
            try {
                Breadboard.placeIC(item.ic, item.slot);
            } catch(e) {
                console.warn(`Could not pre-place IC ${item.ic} in slot ${item.slot}:`, e);
            }
        });
        Simulator.simulate();
    }, 500);
