...
var Simulator = (function() {
    ...
    function simulate() {
        ...
        // Collect source signals (these never change during simulation)
        var sources = {};

        var switchStates = Components.getAllSwitchStates();
        for (var s = 0; s < switchStates.length; s++) {
            sources['switch-' + s] = switchStates[s];
        }

        applyFixedPowerSources(sources);

        // Set clock output
        if (clockEnabled) {
            sources[clockOutputConn] = clockState;
            // Backward compatibility for older saved wires.
            sources['clock-output'] = clockState;
        }

        // Manual pulser dedicated outputs
        sources['pulser-l'] = pulserLState;
        sources['pulser-h'] = pulserHState;

        // Track IC output values across passes (updated each pass)
        var icOutputs = {};

        // Propagate signals through wires to IC inputs (multiple passes for cascaded ICs)
        var maxPasses = ics.length + 2;
        for (var pass = 0; pass < maxPasses; pass++) {
            // Rebuild signals from scratch: sources + latest IC outputs
            var signals = {};
            var k;
            for (k in sources) {
                if (sources.hasOwnProperty(k)) signals[k] = sources[k];
            }
            for (k in icOutputs) {
                if (icOutputs.hasOwnProperty(k)) signals[k] = icOutputs[k];
            }
            ...
            // Capture IC output values for next pass
            for (var j = 0; j < ics.length; j++) {
                var icPlacement = ics[j];
                var pinout = icPlacement.icDef.pinout;
                for (var p = 0; p < pinout.length; p++) {
                    if (pinout[p].type === 'output') {
                        var pinNum = pinout[p].pin;
                        var connId = icPlacement.pins[pinNum];
                        if (signals[connId] !== undefined) {
                            icOutputs[connId] = signals[connId];
                        }
                    }
                }
            }
        ...
        // Final pass: rebuild and propagate one last time
        var finalSignals = {};
        var fk;
        for (fk in sources) {
            if (sources.hasOwnProperty(fk)) finalSignals[fk] = sources[fk];
        }
        for (fk in icOutputs) {
            if (icOutputs.hasOwnProperty(fk)) finalSignals[fk] = icOutputs[fk];
        }
        propagateSignals(finalSignals);

        // Update LED outputs
        for (var l = 0; l < Components.NUM_LEDS; l++) {
            var ledConn = 'led-' + l;
            var ledValue = finalSignals[ledConn] !== undefined ? finalSignals[ledConn] : 0;
            Components.setLED(l, ledValue);
        }

        // Update seven-segment displays from signals
        updateSevenSegments(finalSignals);

        // L12/L13 reflect actual pulser node values after propagation.
        updatePulserLEDs(finalSignals);
    }
    ...
    function probeValue(connId) {
        ...
        for (var pass = 0; pass < maxPasses; pass++) {
            ...
            for (var j = 0; j < ics.length; j++) {
                ...
                for (var p = 0; p < pinout.length; p++) {
                    if (pinout[p].type === 'output') {
                        var pinNum = pinout[p].pin;
                        var cid = icPlacement.pins[pinNum];
                        if (signals[cid] !== undefined) {
                            icOutputs[cid] = signals[cid];
                        }
                    }
                }
            }
        ...
        if (finalSignals[connId] !== undefined) {
            return finalSignals[connId]; // 0 or 1
        }
